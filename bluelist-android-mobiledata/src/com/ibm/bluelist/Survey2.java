...
public class Survey2 extends Activity{
	...
	ArrayAdapter<Item2> lvArrayAdapter;
	ActionMode mActionMode = null;
	int listItemPosition;
	Button next;
	public static final String CLASS_NAME="Survey2";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		itemList = blApplication.getItemList2();
		
		/* Set up the array adapter for items list view. */
		ListView itemsLV = (ListView)findViewById(R.id.itemsList);
		lvArrayAdapter = new ArrayAdapter<Item2>(this, R.layout.list_item_1, itemList);
		...
		itemsLV.setOnItemLongClickListener(new OnItemLongClickListener() {
		    ...
		    public boolean onItemLongClick(AdapterView<?> adapter, View view, int position,
		        ...
		        mActionMode = Survey2.this.startActionMode(mActionModeCallback);
		        return true;
		    }
		});
	...
	public void listItems() {
		try {
			IBMQuery<Item2> query = IBMQuery.queryForClass(Item2.class);
			// Query all the Item objects from the server.
			query.find().continueWith(new Continuation<List<Item2>, Void>() {

				@Override
				public Void then(Task<List<Item2>> task) throws Exception {
                    final List<Item2> objects = task.getResult();
	 ...
	 */
	private void sortItems(List<Item2> theList) {
		// Sort collection by case insensitive alphabetical order.
		Collections.sort(theList, new Comparator<Item2>() {
			public int compare(Item2 lhs,
					Item2 rhs) {
	...
	private ActionMode.Callback mActionModeCallback = new ActionMode.Callback() {
	    ...
	    public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
	    	Item2 lItem = itemList.get(listItemPosition);
